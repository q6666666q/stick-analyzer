...
BUTTON_OPTIONS = [
    ...
    ("X 键 (X)", "X"),
    ("Y 键 (Y)", "Y"),
    ("方向键上 (DPAD_UP)", "DPAD_UP"),
    ("方向键下 (DPAD_DOWN)", "DPAD_DOWN"),
    ("方向键左 (DPAD_LEFT)", "DPAD_LEFT"),
    ("方向键右 (DPAD_RIGHT)", "DPAD_RIGHT"),
    ("左肩键 (LB)", "LEFT_SHOULDER"),
    ("右肩键 (RB)", "RIGHT_SHOULDER"),
    ("左扳机 (LT)", "TRIGGER_LEFT"),
    ("右扳机 (RT)", "TRIGGER_RIGHT"),
    ("左摇杆按下 (L3)", "LEFT_THUMB"),
    ("右摇杆按下 (R3)", "RIGHT_THUMB"),
    ("START", "START"),
    ("BACK / SELECT", "BACK"),
]
...
class StickRecorder:
    ...
    def _run(self):
        XInput = _import_xinput()
        if XInput is None:
            self.on_done(False, "XInput-Python 库未安装，请先 pip install XInput-Python")
            return
        ...
        try:
            ...
            self.on_done(False, f"XInput 调用失败: {e}")
            return

        if controller_idx is None:
            self.on_done(False, "未检测到任何手柄，请连接手柄后重试")
            return
        ...
        try:
            while not self._stop_flag:
                ...
                # FIRE 按键判断
                if self.fire_button == "TRIGGER_LEFT":
                    fire = lt > TRIGGER_THRESHOLD
                elif self.fire_button == "TRIGGER_RIGHT":
                    fire = rt > TRIGGER_THRESHOLD
                else:
                    fire = bool(buttons.get(self.fire_button, False))

                # ADS 按键判断
                if self.ads_button == "TRIGGER_LEFT":
                    ads = lt > TRIGGER_THRESHOLD
                elif self.ads_button == "TRIGGER_RIGHT":
                    ads = rt > TRIGGER_THRESHOLD
                else:
                    ads = bool(buttons.get(self.ads_button, False))
                ...
                if (now_ns - last_update_ns) / 1e9 > 0.2:
...
class App(tk.Tk):
    ...
    def _build_record_tab(self, parent):
        ...
        fire_combo = ttk.Combobox(
            key_frame,
            values=[opt[0] for opt in BUTTON_OPTIONS],
            state="readonly", width=30)
        for i, (label, val) in enumerate(BUTTON_OPTIONS):
            if val == DEFAULT_FIRE_BUTTON:
                fire_combo.current(i)
                break
        ...
        ads_combo = ttk.Combobox(
            key_frame,
            values=[opt[0] for opt in BUTTON_OPTIONS],
            state="readonly", width=30)
        for i, (label, val) in enumerate(BUTTON_OPTIONS):
            if val == DEFAULT_ADS_BUTTON:
                ads_combo.current(i)
                break
    ...
    def _on_button_change(self, combo, btn_type):
        idx = combo.current()
        if idx >= 0:
            label, value = BUTTON_OPTIONS[idx]
            if btn_type == "fire":
                self.fire_button_var.set(value)
            else:
                self.ads_button_var.set(value)
