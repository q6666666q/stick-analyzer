...
- **我的曲线模式**：______（这一项很重要，不同手柄设计不一样）
  - [ ] 模式 X：**腰射 + 开镜分开两套曲线**（北通、飞智、莱仕达、动感指尖等大多支持）
  - [ ] 模式 Y：**只有一条综合曲线**（所有状态共用，不区分腰射和开镜）

## 二、我当前的曲线配置

【根据你上面"曲线模式"和"编辑方式"的选择，按对应模板填写。**没有的曲线类型直接删掉那一段**。】
...
【特别说明】
- 如果你只有一条综合曲线（模式 Y），请说明你的痛点主要发生在「腰射时」还是「开镜时」
  例如："开镜远程压枪过冲，但腰射感觉还行" → AI 调参时会优先优化开镜场景
...
1. **严格匹配我的曲线模式**：
   - 如果我只有「模式 Y：综合曲线」，**只输出一条曲线**，不要给两条
   - 如果是「模式 X：腰射 + 开镜分开」，输出两条
2. **综合分析报告里的数据 + 我的体感痛点 + 参考曲线**，告诉我曲线哪段需要调整
3. **严格按照我的曲线点数限制**给出修改方案（如果我只有 4 个点，就别给 8 个点的方案）
4. **根据我的曲线编辑方式输出**：
   - 如果我选了方式 A：输出完整 JSON
   - 如果我选了方式 B：列出每个节点的 X、Y 值（精确到小数点后 1 位），方便我手动输入
5. **如果是综合曲线模式（模式 Y）**：
   - 综合曲线既要兼顾腰射也要兼顾开镜，是一种折中
   - 调整时要权衡两个场景的需求，告诉我调整后的曲线在哪个场景表现更好
   - 如果两个场景需求冲突，优先满足我体感痛点里更突出的那个
6. **如果我提供了反曲线数学建议，把它当作参考而不是绝对答案**——数学上的反函数不一定是体感最佳
7. 解释为什么这样改，以及预期会改善什么
...
- **绝对不要假设我有几条曲线**——按我在【一、手柄信息】里勾选的"曲线模式"严格匹配
- **关于 RC（动感）**：
